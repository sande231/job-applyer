...
import { FileText, Search, Wand2, ClipboardList, User, X, ChevronDown } from 'lucide-react';
            ...
            {resumeData ? (
              ...
              >
                <div className="w-7 h-7 rounded-full bg-sky-600 flex items-center justify-center text-white text-xs font-bold">
                  {initials(resumeData.name)}
                </div>
                <span className="text-sm font-medium text-gray-700 max-w-[120px] truncate">{resumeData.name}</span>
                <ChevronDown className="w-3.5 h-3.5 text-gray-400" />
              </button>
            ) : (
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-dashed border-gray-300 text-gray-400 text-sm">
              ...
              <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-lg border border-gray-200 p-4 z-20">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900 text-sm">{resumeData.name}</p>
                ...
                {resumeData.email && (
                  <p className="text-xs text-gray-500 mb-3">{resumeData.email}</p>
                )}
                ...
                <div className="border-t border-gray-100 pt-3 flex items-center justify-between">
                  ...
                  <button
                    onClick={() => { clearResume(); setOpen(false); }}
